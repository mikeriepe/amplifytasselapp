...
export default function SiteAnalyticsUpdateForm(props) {
  const {
    id: idProp,
    siteAnalytics: siteAnalyticsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = siteAnalyticsRecord
      ? { ...initialValues, ...siteAnalyticsRecord }
      : initialValues;
    setMonthlySignups(cleanValues.monthlySignups ?? []);
    setCurrentMonthlySignupsValue("");
    setDailySignups(cleanValues.dailySignups ?? []);
    setCurrentDailySignupsValue("");
    setMonthlyPopularEvents(cleanValues.MonthlyPopularEvents ?? []);
    setCurrentMonthlyPopularEventsValue("");
    setMonthlyPopularTags(cleanValues.MonthlyPopularTags ?? []);
    setCurrentMonthlyPopularTagsValue("");
    setMonthlyUserTasselTime(cleanValues.monthlyUserTasselTime);
    setMonthlyUserVolunteerTime(cleanValues.monthlyUserVolunteerTime);
    setMonthlyUserApps(cleanValues.monthlyUserApps);
    setMonthlyNoShows(cleanValues.monthlyNoShows);
    setErrors({});
  };
  const [siteAnalyticsRecord, setSiteAnalyticsRecord] = React.useState(
    siteAnalyticsModelProp
  );
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(SiteAnalytics, idProp)
        : siteAnalyticsModelProp;
      setSiteAnalyticsRecord(record);
    };
    queryData();
  }, [idProp, siteAnalyticsModelProp]);
  React.useEffect(resetStateValues, [siteAnalyticsRecord]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            SiteAnalytics.copyOf(siteAnalyticsRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "SiteAnalyticsUpdateForm")}
      {...rest}
    >
      <ArrayField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || siteAnalyticsModelProp)}
