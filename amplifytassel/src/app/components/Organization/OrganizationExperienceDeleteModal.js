...
export default function OrganizationExperienceDeleteModal({onClose}) {
  ...
  const updateProfile = (index) => {
    ...
    const organizationExperienceObj = [...(userProfile.organizationExperience)];
    organizationExperienceObj.splice(index, 1);

    DataStore.query(Profile, userProfile.id)
      .then((profile) => {
        console.log(JSON.stringify(profile.organizationExperience[index]));
        DataStore.save(Profile.copyOf(profile, updated => {
          updated.organizationExperience = organizationExperienceObj;
        }))
          .then(() => {
            console.log('organization experience updated');
            const userProfileCpy = {...userProfile};
            userProfileCpy.organizationExperience = organizationExperienceObj;
  ...
  return (
    ...
    >
      ...
      >
        Which organization activity would you like to delete?
      </MuiBox>
        ...
        <div className='grid-flow-small'>
          <div className='flex-flow-large'>
            <ThemedButton
              ...
              aria-label='Organization Experience Delete Modal Cancel Button'
              color={'yellow'}
              variant={'themed'}
              sx={{
                marginRight: '10px',
